...
<div class="row mt-4">
    ...
    <aside class="col-md-3 mb-4">
        <div class="list-group card border-dark shadow-sm">
            ...
            <?= $this->Html->link(
                '<i class="fa-solid fa-list me-2"></i> List of Rentals',
    ...
    <div class="col-md-9">
        <div class="card border-dark shadow-sm mb-5">
            <div class="card-body">
                ...
                <?= $this->Form->create($rental, ['type' => 'post']) ?>

                <h4 class="mb-4">EDIT RENTAL FORM</h4>
                ...
                <div class="row g-3 mt-1">
                     <div class="col-md-6">
                        <?= $this->Form->control('doc_text', [
                            'label' => 'Document Text',
                            'class' => 'form-control',
                            'placeholder' => 'Example: SR_DEC 2025 - JAN 2026'
                        ]) ?>
                    </div>
                    <div class="col-md-6">
                        <?= $this->Form->control('description', [
                            ...
                            'class' => 'form-control',
                            'placeholder' => 'Example: SR_DEC 2025 - JAN 2026_8099P'
                        ]) ?>
                    </div>
                </div>
                ...
                <div class="table-responsive">
                    <table class="table table-bordered align-middle">
                        ...
                        <tbody id="items">
                            <?php foreach ($rental->rental_items as $i => $item): ?>
                                <tr class="item-row">
                                    <td><?= $this->Form->control("rental_items.$i.reference", ['label' => false, 'class' => 'form-control', 'value' => $item->reference]) ?></td>
                                    <td><?= $this->Form->control("rental_items.$i.doc_text", ['label' => false, 'class' => 'form-control', 'value' => $item->doc_text]) ?></td>
                                    <td><?= $this->Form->control("rental_items.$i.amount", ['label' => false, 'class' => 'form-control', 'value' => $item->amount]) ?></td>
                                    <td><?= $this->Form->control("rental_items.$i.order_number", ['label' => false, 'class' => 'form-control', 'value' => $item->order_number]) ?></td>
                                    <td><?= $this->Form->control("rental_items.$i.description", ['label' => false, 'class' => 'form-control', 'value' => $item->description]) ?></td>
                                    <td class="text-center">
                                        <button type="button" class="btn btn-sm btn-danger remove-item">✕</button>
                                    </td>

                                    <?= $this->Form->hidden("rental_items.$i.id", ['value' => $item->id]) ?>
                                    <?= $this->Form->hidden("rental_items.$i.line_no", ['value' => $item->line_no]) ?>
                                    <?= $this->Form->hidden("rental_items.$i.status", ['value' => $item->status]) ?>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
...
<script>
let index = <?= count($rental->rental_items) ?>; // start after existing rows
...
document.getElementById('add-item')?.addEventListener('click', function () {
    ...
    row.innerHTML = `
        ...
        <input type="hidden" name="rental_items[${index}][line_no]" value="${index + 1}">
